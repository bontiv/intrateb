...
<tbody>
  {foreach from=$es item="i"}
      <tr>
        ...
        <td><div class="{if $i.staffs->count()>$i.es_needed}text-danger{elseif $i.staffs->count()==$i.es_needed}text-success{/if}">{$i.staffs->count()} / {$i.es_needed}</<div>{if $i.cdat} <span class="label label-warning">Candidat</span>{/if}</td>
              <td>
